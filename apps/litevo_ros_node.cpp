// =============================================================================
// LiteVO ROS2 Node — real-time monocular SLAM for ROS2
// =============================================================================
//
// Subscribes to sensor_msgs/Image, runs the LiteVO tracking pipeline,
// and publishes:
...
//   - TF: odom → camera_link
//
// Build: -DLITEVO_BUILD_ROS2=ON (requires ROS2 Humble/Iron + cv_bridge + tf2_ros)
// =============================================================================
...
public:
    LiteVONode() : Node("litevo_slam") {
        ...
        // ── Create tracker ──────────────────────────────────────────────
        tracker_ = std::make_unique<litevo::tracking::Tracker>(camera_, cfg.tracking, cfg.orb);

        // ── Create local mapper ─────────────────────────────────────────
        litevo::features::OrbExtractor::Options mapper_opts;
        ...
        mapper_extractor_ = std::make_unique<litevo::features::OrbExtractor>(mapper_opts);
        ...
        image_sub_ = this->create_subscription<sensor_msgs::msg::Image>(
            ...
            std::bind(&LiteVONode::ImageCallback, this, std::placeholders::_1));
...
private:
    ...
    void PublishTF(const litevo::SE3& Tcw, const rclcpp::Time& stamp) {
        litevo::SE3 Twc = Tcw.inverse();
        litevo::Pose p = litevo::Pose::FromSE3(Twc);
    ...
    void PublishKeyFrameMarkers(const rclcpp::Time& stamp) {
        ...
        for (const auto& kf : keyframes) {
            ...
            litevo::Vec3 center = kf->CameraCenter();
            litevo::SE3 Twc = kf->Pose().inverse();
            litevo::Mat3 R = Twc.rotation();

            // Four corners of a virtual frustum at 1m distance
            double f = 0.5;  // half-size of frustum face
            litevo::Vec3 corners[4] = {
                Twc * litevo::Vec3(f, f, 1.0),
                Twc * litevo::Vec3(-f, f, 1.0),
                Twc * litevo::Vec3(-f, -f, 1.0),
                Twc * litevo::Vec3(f, -f, 1.0),
            };
...
int main(int argc, char* argv[]) {
    ...
    auto node = std::make_shared<litevo_ros::LiteVONode>();
    rclcpp::spin(node);
    rclcpp::shutdown();
    return 0;
}

#else  // LITEVO_HAS_ROS2

#include <cstdio>

int main() {
    std::fprintf(stderr, "Error: LiteVO was built without ROS2 support.\n");
    std::fprintf(stderr, "Rebuild with: -DLITEVO_BUILD_ROS2=ON\n");
    return 1;
}

#endif  // LITEVO_HAS_ROS2
