...
float MapPoint::GetFoundRatio() const {
    if (visible_count_ == 0) return -1.0f;
...
bool MapPoint::IsEraseReady(int min_observations) const {
    ...
    if (observations_ >= min_observations) return false;

    // For recently created points (first 2 frames), don't erase yet
    if (frames_since_creation_ <= 2) return false;

    if (found_count_ == 0) return true;

    if (visible_count_ > 0) {
        float ratio = GetFoundRatio();
        if (ratio >= 0.0f && ratio < 0.25f) return true;
    }
