...
public:
    ...
    [[nodiscard]] std::vector<std::pair<int, int>> MatchByDescriptor(const cv::Mat& desc1,
                                                                     const cv::Mat& desc2,
                                                                     float ratio_threshold = 0.7f,
                                                                     bool cross_check = true) const;
    ...
    int SearchForInitialization(const Frame& F1, const Frame& F2, int window_size,
    ...
    [[nodiscard]] std::vector<std::pair<int, MapPointId>> SearchByProjection(
        Frame& frame, const std::vector<std::shared_ptr<MapPoint>>& map_points,
        const Camera& camera, float max_dist_px = 4.0f, int max_desc_dist = 50) const;
